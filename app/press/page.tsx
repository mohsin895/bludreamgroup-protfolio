    ...
    <div style={{ display: "flex", gap: "3px", alignItems: "center" }}>
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          ...
          style={{
            color: i < count ? "#C9A84C" : "rgba(255,255,255,0.2)",
          }}
        />
      ))}
    </div>
            ...
            >
              {[
                { label: "Press Pieces", value: PRESS_ITEMS.length },
                {
                  ...
                  value: PRESS_ITEMS.filter((p) => p.stars === 5).length,
                },
                {
                  label: "Publications",
                  value: new Set(PRESS_ITEMS.map((p) => p.publication)).size,
                },
              ].map((s) => (
                <div key={s.label}>
                  <div
