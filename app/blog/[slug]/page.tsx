...
function HeroImage({ url, title }: { url: string | null; title: string }) {
    ...
    return (
        <div style={{ width: "100%", height: "320px", overflow: "hidden", marginBottom: "48px", borderRadius: "2px" }}>
            <Image
                src={url}
                alt={title}
                width={800}
...
function WriterAvatar({ writer, size = 38 }: { writer: MappedWriter; size?: number }) {
    const [err, setErr] = useState(false);
    ...
    return (
        <div
            style={{
                ...
                background: showImg ? "transparent" : "linear-gradient(135deg,#b8933a,#7a5f1e)",
                display: "flex", alignItems: "center", justifyContent: "center",
                overflow: "hidden", flexShrink: 0,
                fontSize: `${Math.round(size * 0.33)}px`, fontWeight: 600, color: "#fff", letterSpacing: "0.05em",
            }}
        >
            {showImg ? (
                <Image
...
function RelatedCard({ post, i }: { post: MappedBlog; i: number }) {
    return (
        <AnimatedSection delay={i * 0.08}>
            <Link href={`/blog/${post.slug}`} className="related-card" style={{ display: "flex" }}>
...
export default function BlogDetailPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setLoading(true);
                setError(null);
                const [p, all] = await Promise.all([fetchBlogBySlug(slug), fetchBlogs()]);
                if (!p) { setError("Post not found."); return; }
                setPost(p);
                // Related: same category, different post, max 2
                setRelated(all.filter((b) => b.id !== p.id && b.category === p.category).slice(0, 2));
            } catch (err) {
    ...
    return (
        ...
        .detail-hero { background:var(--ink); padding:160px 24px 0; position:relative; overflow:hidden; }
        .detail-hero::before {
        ...
        .article-wrap { background:var(--parchment); padding:72px 24px 80px; }
        .article-inner { max-width:var(--reading-col); margin:0 auto; }
        .article-p { font-family:'DM Sans',sans-serif; font-size:18px; font-weight:300; line-height:1.85; color:#2a2720; margin:0 0 28px; }
        .article-h2 { font-family:'Playfair Display',serif; font-size:26px; font-weight:600; color:var(--ink); margin:52px 0 20px; line-height:1.25; }
        ...
        .share-label { font-size:12px; letter-spacing:0.12em; text-transform:uppercase; color:var(--muted); font-weight:500; }
        .share-btns { display:flex; gap:8px; flex-wrap:wrap; }
        .share-btn { display:inline-flex; align-items:center; gap:7px; padding:8px 16px; background:transparent; border:1px solid var(--border); border-radius:2px; font-size:12px; font-weight:500; color:var(--muted); font-family:'DM Sans',sans-serif; cursor:pointer; letter-spacing:0.05em; transition:all 0.18s; text-decoration:none; }
        .share-btn:hover { border-color:var(--ink); color:var(--ink); }

        .author-card { max-width:var(--reading-col); margin:64px auto 0; background:var(--ink); border-radius:4px; padding:40px 44px; display:flex; gap:28px; align-items:flex-start; }
        ...
        .author-card-role { font-size:12px; letter-spacing:0.1em; text-transform:uppercase; color:var(--gold); margin-bottom:14px; }
        .author-card-bio { font-size:14px; color:rgba(255,255,255,0.45); line-height:1.75; font-weight:300; margin:0; }
        ...
        .related-section { background:var(--ink); padding:80px 24px; }
        .related-inner { max-width:1000px; margin:0 auto; }
        .related-eyebrow { font-size:10px; font-weight:500; letter-spacing:0.2em; text-transform:uppercase; color:var(--gold); margin-bottom:32px; display:flex; align-items:center; gap:10px; }
        .related-eyebrow::before { content:''; display:inline-block; width:28px; height:1px; background:var(--gold); }
        .related-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:1px; background:rgba(255,255,255,0.06); }
        ...
        .related-card-tag { font-size:10px; font-weight:500; letter-spacing:0.15em; text-transform:uppercase; color:var(--gold); background:rgba(184,147,58,0.1); padding:3px 8px; border-radius:2px; width:fit-content; }
        .related-card-title { font-family:'Playfair Display',serif; font-size:18px; font-weight:600; color:#fff; line-height:1.3; flex:1; }
        .related-card-meta { font-size:12px; color:rgba(255,255,255,0.25); display:flex; gap:12px; }
        .related-card-arrow { width:26px; height:26px; border-radius:50%; border:1px solid rgba(255,255,255,0.12); display:flex; align-items:center; justify-content:center; font-size:13px; color:rgba(255,255,255,0.3); transition:all 0.2s; align-self:flex-end; }
        .related-card:hover .related-card-arrow { background:var(--gold); color:#fff; border-color:var(--gold); }
      ...
      `}</style>
            ...
            {/* Reading bar */}
            <div id="reading-bar-track"><div id="reading-bar" /></div>
            ...
            {/* ── Error ── */}
            {!loading && error && (
                <div style={{ minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "16px", background: "var(--parchment)", padding: "40px 24px", textAlign: "center" }}>
                    <AlertCircle size={36} style={{ color: "#c0392b", opacity: 0.6 }} />
                    <p style={{ color: "var(--muted)", fontSize: "16px", margin: 0 }}>{error}</p>
                    <Link href="/blog" style={{ fontSize: "13px", color: "var(--ink)", fontWeight: 500, textDecoration: "underline" }}>← Back to the Journal</Link>
                </div>
            )}
                    ...
                    <section className="detail-hero">
                        <div className="hero-inner">
                            <AnimatedSection>
                                ...
                                {post.excerpt && <p className="detail-subtitle">{post.excerpt}</p>}

                                <div className="detail-meta-row">
                                    {primaryWriter && (
                                        <>
                                            <div className="author-chip">
                                                <WriterAvatar writer={primaryWriter} size={38} />
                                                <div>
                                                    <div className="author-name">{primaryWriter.name}</div>
                                                    <div className="author-role">{primaryWriter.designation}</div>
                                                </div>
                                            </div>
                            ...
                            <div className="article-inner">
                                <AnimatedSection>
                                    ...
                                    {post.description ? (
                                        <div
                                            className="article-p"
                                            style={{ fontSize: "18px", fontWeight: 300, lineHeight: 1.85, color: "#2a2720" }}
                                            dangerouslySetInnerHTML={{ __html: post.description }}
                                        />
                                    ) : (
                                        <p className="article-p" style={{ color: "var(--muted)", fontStyle: "italic" }}>
                                    ...
                                    {/* Tags */}
                                    {post.tags.length > 0 && (
                                        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", margin: "32px 0" }}>
                                            {post.tags.map((tag) => (
                                                <span
                                                    key={tag}
                                                    style={{ fontSize: "11px", fontWeight: 500, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--gold)", background: "var(--gold-light)", padding: "4px 10px", borderRadius: "2px" }}
                                                >
                          {tag}
                        </span>
                                            ))}
                                ...
                                {primaryWriter && (
                                    <div className="author-card">
                                        ...
                                        <div style={{ flex: 1 }}>
                                            ...
                                            <div className="author-card-role">{primaryWriter.designation}</div>
                                            {primaryWriter.description && (
                                                <p className="author-card-bio">{primaryWriter.description}</p>
                                            )}
                                            {(primaryWriter.twitter || primaryWriter.linkedin) && (
                                                <div className="author-card-social">
                                                    {primaryWriter.twitter  && <a href={primaryWriter.twitter}  target="_blank" rel="noopener noreferrer" className="author-social-link">Twitter</a>}
                                                    {primaryWriter.linkedin && <a href={primaryWriter.linkedin} target="_blank" rel="noopener noreferrer" className="author-social-link">LinkedIn</a>}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <section className="related-section">
                            <div className="related-inner">
                                ...
                                <div className="related-grid">
                                    {related.map((r, i) => <RelatedCard key={r.id} post={r} i={i} />)}
                                </div>
                            </div>
                        </section>
                    )}
