...
export function lowestPrice(
  ...
  return formats.reduce((prev, curr) => {
    const p =
      typeof prev.price === "string" ? parseFloat(prev.price) : prev.price;
    const c =
      typeof curr.price === "string" ? parseFloat(curr.price) : curr.price;
...
export async function getAllProducts(): Promise<Product[]> {
  try {
    ...
    if (Array.isArray((raw as ProductsResponse).data))
      return (raw as ProductsResponse).data!;
    if (Array.isArray((raw as ProductsResponse).products))
      return (raw as ProductsResponse).products!;
    return [];
  } catch {
    return [];
  }
...
export async function getProductBySlug(
  ...
  if (
    envelope?.product &&
    typeof envelope.product === "object" &&
    envelope.product.id
  ) {
    // { data: { product: {...}, related_products: [] } }
  ...
  try {
    ...
    const all = Array.isArray(raw)
      ...
      : ((raw as ProductsResponse).data ??
        (raw as ProductsResponse).products ??
        []);
